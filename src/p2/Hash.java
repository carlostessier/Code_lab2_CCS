package p2;

import org.bouncycastle.crypto.digests.GeneralDigest;
import org.bouncycastle.crypto.digests.LongDigest;
import org.bouncycastle.crypto.digests.MD5Digest;
import org.bouncycastle.crypto.digests.SHA1Digest;
import org.bouncycastle.crypto.digests.SHA512Digest;
import org.bouncycastle.util.encoders.Hex;

/**
 * Ejecuta distintas funciones resumen
 */
public class Hash {
	
	/**
	 * Establece la funci贸n resumen MD5 para el procesamiento
	 */
	public void doMD5() {
		doDigest(new MD5Digest());
	}

	/**
	 * Establece la funci贸n resumen SHA1 para el procesamiento
	 */
	public void doSHA1() {
		doDigest(new SHA1Digest());
	}
	
	/**
	 * Establece la funci贸n resumen SHA256 para el procesamiento
	 */
	public void doSHA512() {
		doDigest(new SHA512Digest());
	}
	
	/**
	 * Gestiona un digest de un archivo
	 * @param digest
	 * @return
	 */
	protected byte[] doDigest(LongDigest digest) {
		byte[] fileContent = Utils.instance().doSelectFile(
				"Seleccione un archivo", "txt");
		if (fileContent != null) {
			byte[] result = digest(digest, fileContent);
			System.out.println("El resumen es:" + new String(Hex.encode(result)));
			return result;
		}
		return null;
	}

	/**
	 * Gestiona un digest de un archivo
	 * @param digest
	 * @return
	 */
	protected byte[] doDigest(GeneralDigest digest) {
		byte[] fileContent = Utils.instance().doSelectFile(
				"Seleccione un archivo", "txt");
		if (fileContent != null) {
			byte[] result = digest(digest, fileContent);
			System.out.println("El resumen es:" + new String(Hex.encode(result)));
			return result;
		}
		return null;
	}
	
	

	/**
	 * Realiza el procesamiento de la funci髇 resumen seleccionada
	 * @param digest
	 * @param input
	 * @return
	 */
	public byte[] digest(LongDigest digest, byte[] input) {
		digest.update(input, 0, input.length);

		// get the output/ digest size and hash it
		byte[] result = new byte[digest.getDigestSize()];
		digest.doFinal(result, 0);
		return result;
	}
	
	/**
	 * Realiza el procesamiento de la funci贸n resumen seleccionada
	 * @param digest
	 * @param input
	 * @return
	 */
	public byte[] digest(GeneralDigest digest, byte[] input) {
		digest.update(input, 0, input.length);

		// get the output/ digest size and hash it
		byte[] result = new byte[digest.getDigestSize()];
		digest.doFinal(result, 0);
		return result;
	}

}
